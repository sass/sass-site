...
function showLoader(id) {
  $('#' + id + ' .progress').fadeIn('slow');
}

//hide loading graphic
function hideLoader(id) {
  $('#' + id + ' .progress').fadeOut('slow');
}
...
$(document).ready(function(){
  ...
  $('.responsive-test ul').css('width', function(){
    var width = 0;
  ...
  $('input[type=radio]').change(function(){
    $frames = $('.responsive-test');
    $inputs = $('input[type=radio]:checked').val();

    if($inputs == '1'){
      $frames.addClass('width-only');
    } else {
      $frames.removeClass('width-only');
    }
  });
